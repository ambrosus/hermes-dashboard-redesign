...
export const timestampToDateString = (ts) =>
  moment.unix(ts).format('MMMM Do YYYY, h:mm:ss a');

export const getTimestampMonthStart = () => moment().startOf('M').unix();
...
export const getTimestampSubWeeks = (w) => moment().subtract(w, 'weeks').unix();

export const getTimestampSubMonths = (m) =>
  moment().subtract(m, 'months').unix();

export const getTimestampAddDays = (d) => moment().add(d, 'days').unix();
